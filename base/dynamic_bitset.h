...
#include "base/dynamic_bitset_detail.h"
...
public:
    ...
    :m_bits(alloc),
     m_num_bits(0)
    {
        using base::detail::dynamic_bitset_impl::value_to_type;
        using base::detail::dynamic_bitset_impl::is_numeric;
    ...
    void m_append(BlockInputIterator first, BlockInputIterator last, std::forward_iterator_tag)
    {
        ...
        std::size_t d = base::detail::distance(first, last);
...
to_ulong() const
{
  ...
  if (find_next(ulong_width - 1) != npos)
    throw(std::overflow_error("base::dynamic_bitset::to_ulong overflow"));
...
dynamic_bitset<Block, Allocator>::m_do_find_from(size_type first_block) const
{
    ...
    return i * bits_per_block + static_cast<size_type>(std::lowest_bit(m_bits[i]));
...
bool operator<(const dynamic_bitset<Block, Allocator>& a,
               const dynamic_bitset<Block, Allocator>& b)
{
    typedef dynamic_bitset<Block, Allocator>::size_type size_type;
           ...
           const dynamic_bitset<Block, Alloc>& b)
{
    ...
    if (cerberos) {
		...
		const std::ctype<Ch> & fac = std::use_facet<std::ctype<Ch>>(os.getloc());
        const Ch zero = fac.widen('0');
        const Ch one  = fac.widen('1');
...
operator>>(std::basic_istream<Ch, Tr>& is, dynamic_bitset<Block, Alloc>& b)
{
    ...
    if(cerberos) {
		...
		const std::ctype<Ch> & fac = std::use_facet<std::ctype<Ch>>(is.getloc());
		const Ch zero = fac.widen('0');
		const Ch one = fac.widen('1');
